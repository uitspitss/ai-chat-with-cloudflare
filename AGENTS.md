...
**開発ルールの実体はこのファイル（`AGENTS.md`）。** `CLAUDE.md` は `@AGENTS.md`
の 1 行だけで、ここを読み込むためにある。Claude 以外のエージェントも同じルールを
読めるようにするための構成なので、**`CLAUDE.md` にルールを書き足さない**
（片方だけ育って、どちらが正なのか分からなくなる）。

ドメインの語彙（Thread / Attachment / Message / ChatAgent / 履歴消去 / Session / User）は
`CONTEXT.md` にある。**新しい名前を付ける前に読む。**
...
**「完了」= lint / format:check / typecheck / test が通り、変更が動くことを
実際に確認した状態。** テストを書かずに「動くはず」と報告しない。
落ちたテストは報告に含める。

lefthook が pre-commit で lint / format / typecheck、pre-push で knip を回す。
手で流すのを飛ばしてもコミット時に捕まるが、**捕まってから直すと差分が汚れる**ので先に流す。
...
`apps/web` の `nr test` は vitest の projects が 2 つある。**入口が 2 つあり、
どちらを壊しても「0 件で緑」になる**（`apps/web/vitest.config.ts` にも書いてある）。

- `unit` — `src/**/*.{test,spec}.{ts,tsx}`（happy-dom）
- `storybook` — `.storybook/main.ts` の `stories` glob を addon-vitest が
  実ブラウザ（playwright / chromium）で走らせる

コンポーネントを足したら `.stories.tsx` を書く。**増減させていないのに件数が
減っていたら glob が壊れている**ので、通ったことではなく件数を見る。
`nr storybook` で :6006 に上がる。
...
- durable-objects - ChatAgent が乗っている DO
- hono - サーバーのルーティング
- cloudflare / wrangler - Cloudflare プラットフォーム
- expo:expo-router - mobile のファイルベースルーティング
- expo:expo-tailwind-setup - NativeWind v5 / react-native-css の結線
- frontend-design - フロントエンド UI 作成

**TanStack Router / Query のスキルは存在しない。** 代わりに `find-docs`
（context7）で最新のドキュメントを引く。訓練データの記憶で API を書かない。
